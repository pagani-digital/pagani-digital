...
// ???? AUTH ??????????????????????????????????????????????????????????????????????????????????????????????????????
async function apiRegisterStart({ name, email, password, refCode, mmPhone, mmOperator, mmName }) {
  return _fetch('/auth/register/start', { method: 'POST', body: JSON.stringify({ name, email, password, refCode, mmPhone, mmOperator, mmName }) });
}
async function apiRegisterVerify({ email, code }) {
  const d = await _fetch('/auth/register/verify', { method: 'POST', body: JSON.stringify({ email, code }) });
  _setToken(d.token);
  return d.user;
}
...
async function apiGetWithdraws()        { return _fetch('/withdraws'); }
...
window.PaganiAPI = {
  // Auth
  registerStart:   apiRegisterStart,
  registerVerify:  apiRegisterVerify,
  login:           apiLogin,
